...
N_PEN_DOWN_DELAY = 400
# delay (ms) for the pen to up down before the next move
N_PEN_UP_DELAY = 400
...
class WCB(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.PenDownSpeed = 0.25 * axidraw_conf.Speed_Scale
        # Default speed when pen is down
        self.PenUpSpeed = 0.75 * axidraw_conf.Speed_Scale
    ...
    def effect(self):
        '''Main entry point: check to see which tab is selected, and act
        accordingly.'''
    ...
    def plotDocument(self):
        ...
        # parse the svg data as a series of line segments and send each segment
        # to be plotted
        ...
        try:
            ...
            if (not self.bStopped):
                if (self.options.tab == '"splash"') or (self.options.tab == '"layers"') or (self.options.tab == '"resume"'):
                    ...
                    # IF we have completed a normal plot from the splash,
                    # layer, or resume tabs.
    ...
    def recursivelyTraverseSvg(self, aNodeList,
        ...
        for node in aNodeList:
            ...
            if node.tag == inkex.addNS('g', 'svg') or node.tag == 'g':
                ...
                # if we're in resume mode and self.pathcount = self.svgLastPath,
                # then start here, and set self.nodeCount equal to
                # self.svgLastPathNC
                ...
                if (doWePlotThisPath):
                    ...
                    #   X1 = CX - RX
                    #   X2 = CX + RX
                    # Note: ellipses or circles with a radius attribute of
                    # value 0 are ignored
    ...
    def DoWePlotLayer(self, strLayerName):
        ...
        CurrentLayerName = string.lstrip(strLayerName.encode(
            'ascii', 'ignore'))  # remove leading whitespace
        ...
        if (str.isdigit(TempNumString)):
            ...
            if (self.svgLayer == layerNameInt):
                # Match! The current layer IS named in the Layers tab.
                layerMatch = True
    ...
    def PlanTrajectory(self, inputPath):
        ...
        # Handle simple segments (lines) that do not require any complex
        # planning:
        if (len(inputPath) < 3):
            if spewTrajectoryDebugData:
        ...
        for i in xrange(1, TrajLength - 1):
            ...
            if (Dcurrent > accelDist):
                ...
                VcurrentMax = plot_utils.vFinal_Vi_A_Dx(
                    VPrevExit, accelRate, Dcurrent)
            ...
            cosineFactor = - \
                plot_utils.dotProductXY(TrajVectors[i - 1], TrajVectors[i])
        ...
        for j in xrange(1, TrajLength):
            ...
            if (Vinitial > Vfinal) and (SegLength > 0):
                VInitMax = plot_utils.vInitial_VF_A_Dx(
        ...
        for i in xrange(1, TrajLength):
            self.plotSegmentWithVelocity(inputPath[i][0], inputPath[i][
    ...
    def plotSegmentWithVelocity(self, xDest, yDest, Vi, Vf):
        ...
        accelDistMax = (initialVel * tAccelMax) + \
            (0.5 * accelRate * tAccelMax * tAccelMax)
        # Use the same model for deceleration distance; modeling it with
        # backwards motion:
        decelDistMax = (finalVel * tDecelMax) + \
            (0.5 * accelRate * tDecelMax * tDecelMax)

        # (seconds): Slice travel into slices of time that are at least 0.050 seconds (50 ms) long
        timeSlice = axidraw_conf.TIME_SLICE

        # This whole segment move counts as ONE pause/resume node in our plot
        self.nodeCount += 1
        ...
        # unsigned integer for duration -- up to 65 seconds for a move if only
        # 2 bytes.
        durationArray = array('I')
        ...
        if (ConstantVelMode == False) or (self.virtualPenIsUp):
            if (plotDistance > (accelDistMax + decelDistMax + timeSlice * speedLimit)):
                ...
                intervals = int(math.floor(tAccelMax / timeSlice))

                # If intervals == 0, then we are already at (or nearly at) full
                # speed.
                if (intervals > 0):
                    timePerInterval = tAccelMax / intervals

                    velocityStepSize = (
                        speedMax - initialVel) / (intervals + 1.0)
                ...
                if (coastingDistance > (timeSlice * speedMax)):
                    ...
                    distArray.append(position)
                    if spewSegmentDebugData:
                        inkex.errormsg('Coast Distance: ' +
                                       str(coastingDistance))

                # Number of intervals during deceleration
                intervals = int(math.floor(tDecelMax / timeSlice))

                if (intervals > 0):
                    timePerInterval = tDecelMax / intervals
                    velocityStepSize = (
                        speedMax - finalVel) / (intervals + 1.0)
                ...
                if (Ta < 0):
                    ...
                    if spewSegmentDebugData:
                        # Should not happen. :)
                        inkex.errormsg(
                            'Warning: Negative transit time computed.')
                ...
                # Number of intervals during acceleration
                Dintervals = int(math.floor(Td / timeSlice))

                if ((intervals + Dintervals) > 4):
                    if (intervals > 0):
                        if spewSegmentDebugData:
                            inkex.errormsg(
                                'Triangle intervals UP: ' + str(intervals))

                        timePerInterval = Ta / intervals
                        velocityStepSize = (
                            Vmax - initialVel) / (intervals + 1.0)
                        # For six time intervals of acceleration, first interval is at velocity (max/7)
                        # 6th (last) time interval is at 6*max/7
                        # after this interval, we are at full speed.
                        ...
                        for index in range(0, intervals):
                            ...
                            timeElapsed += timePerInterval
                            position += velocity * timePerInterval
                            durationArray.append(
                                int(round(timeElapsed * 1000.0)))
                            # Estimated distance along direction of travel
                            distArray.append(position)
                    else:
                        if spewSegmentDebugData:
                            inkex.errormsg(
                                'Note: Skipping accel phase in triangle.')

                    if (Dintervals > 0):
                        if spewSegmentDebugData:
                            inkex.errormsg(
                                'Triangle intervals Down: ' + str(intervals))

                        timePerInterval = Td / Dintervals
                        velocityStepSize = (
                            Vmax - finalVel) / (Dintervals + 1.0)
                        # For six time intervals of acceleration, first interval is at velocity (max/7)
                        # 6th (last) time interval is at 6*max/7
                        # after this interval, we are at full speed.

                        # Calculate acceleration phase
                        for index in range(0, Dintervals):
                            velocity -= velocityStepSize
                            timeElapsed += timePerInterval
                            position += velocity * timePerInterval
                            durationArray.append(
                    ...
                    # So long as this 'a' is less than accelRate, we can
                    # linearly interpolate in velocity.

                    # Boost initial speed for this segment
                    initialVel = (Vmax + initialVel) / 2
                    velocity = initialVel  # Boost initial speed for this segment

                    localAccel = (finalVel * finalVel - initialVel *
                                  initialVel) / (2.0 * plotDistance)
                    ...
                    if (localAccel > accelRate):
                        ...
                        localAccel = -accelRate
                    if (localAccel == 0):
                        # Initial velocity = final velocity -> Skip to constant
                        # velocity routine.
                        ConstantVelMode = True
                    else:
                    ...
                    # Number of intervals during deceleration
                    intervals = int(math.floor(tSegment / timeSlice))
                    if (intervals > 1):
                        timePerInterval = tSegment / intervals
                        velocityStepSize = (
                            finalVel - initialVel) / (intervals + 1.0)
                        # For six time intervals of acceleration, first interval is at velocity (max/7)
                        # 6th (last) time interval is at 6*max/7
                        # after this interval, we are at full speed.
        ...
        if spewSegmentDebugData:
            inkex.errormsg('position/plotDistance: ' +
                           str(position / plotDistance))

        for index in range(0, len(distArray)):
            # Scale our trajectory to the "actual" travel distance that we
            # need:
            # Fractional position along the intended path
            fractionalDistance = distArray[index] / position
        ...
        for index in range(0, len(destArray1)):
            ...
            xSteps = (moveSteps1 + moveSteps2) / 2.0
                ...
                if (not self.resumeMode) and (not self.bStopped):
                    ...
                    if (moveTime > 15):
                        if self.options.tab != '"manual"':
                            ...
                            time.sleep(float(moveTime - 10) / 1000.0)
                    else:
                        if spewSegmentDebugData:
                            inkex.errormsg('ShortMoves: ' +
                                           str(moveTime) + '.')
    ...
    def EnableMotors(self):
        ...
        * Reduce speed by factor of 2 when disabling acceleration
        ...
        if (TestArray.itemsize < 4):
            ...
            # than we are expecting. If anyone ever comes across such a system,
            # we need to know!
