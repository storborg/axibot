...
def formatTransform(mat):
    return ("matrix(%f,%f,%f,%f,%f,%f)" % (mat[0][0], mat[1][0], mat[0][1],
...
def composeParents(node, mat):
    ...
    if node.getparent().tag == inkex_addNS('g', 'svg'):
...
def roughBBox(path):
    xmin, xMax, ymin, yMax = path[0][0][0][0], path[
        0][0][0][0], path[0][0][0][1], path[0][0][0][1]
...
def refinedBBox(path):
    xmin, xMax, ymin, yMax = path[0][0][1][0], path[
        0][0][1][0], path[0][0][1][1], path[0][0][1][1]
    for pathcomp in path:
        for i in range(1, len(pathcomp)):
            cmin, cmax = cubicExtrema(
                pathcomp[i - 1][1][0], pathcomp[i - 1][2][0],
                pathcomp[i][0][0], pathcomp[i][1][0])
            xmin = min(xmin, cmin)
            xMax = max(xMax, cmax)
            cmin, cmax = cubicExtrema(
                pathcomp[i - 1][1][1], pathcomp[i - 1][2][1],
                pathcomp[i][0][1], pathcomp[i][1][1])
            ymin = min(ymin, cmin)
            yMax = max(yMax, cmax)
    return xmin, xMax, ymin, yMax
...
def cubicExtrema(y0, y1, y2, y3):
    ...
    if (d1 - 2 * d2 + d3):
        if (d2 * d2 > d1 * d3):
            ...
            if (t > 0) and (t < 1):
                y = y0 * (1 - t) * (1 - t) * (1 - t) + 3 * y1 * t * (1 - t) * \
                    (1 - t) + 3 * y2 * t * t * (1 - t) + y3 * t * t * t
                cmin = min(cmin, y)
                cmax = max(cmax, y)
            t = (d1 - d2 - math.sqrt(d2 * d2 - d1 * d3)) / (d1 - 2 * d2 + d3)
            if (t > 0) and (t < 1):
...
def computeBBox(aList, mat=[[1, 0, 0], [0, 1, 0]]):
    ...
    for node in aList:
        ...
        if node.get("d"):
            ...
            d = 'M' + node.get('points')
        elif node.tag in [inkex_addNS('rect', 'svg'),
                          'rect',
                          inkex_addNS('image', 'svg'),
                          'image']:
            d = 'M' + node.get('x', '0') + ',' + node.get('y', '0') + \
                'h' + node.get('width') + 'v' + node.get('height') + \
                'h-' + node.get('width')
        elif node.tag in [inkex_addNS('line', 'svg'), 'line']:
            d = 'M' + node.get('x1') + ',' + node.get('y1') + \
                ' ' + node.get('x2') + ',' + node.get('y2')
        elif node.tag in [inkex_addNS('circle', 'svg'), 'circle',
                          inkex_addNS('ellipse', 'svg'), 'ellipse']:
            rx = node.get('r')
            if rx is not None:
                ry = rx
            else:
        ...
        elif node.tag == inkex_addNS('use', 'svg') or node.tag == 'use':
            refid = node.get(inkex_addNS('href', 'xlink'))
            path = '//*[@id="%s"]' % refid[1:]
